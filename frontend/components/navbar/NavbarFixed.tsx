...
export default function NavbarFixed({ toggleTheme, currentTheme }: NavbarFixedProps) {
  ...
  return (
    <div
      className={`fixed top-0 left-0 z-50 w-full transition-all duration-300 ${
        scrolled ? 'bg-white shadow-md dark:bg-zinc-900' : 'bg-transparent'
      }`}
    >
      <div className="container mx-auto flex items-center justify-between px-6 py-6 md:px-12 lg:px-20">
        ...
        <Link href="/" className="flex items-center space-x-3">
          <Image
            src={currentTheme === 'light' ? '/img/light/logo/logo_black.png' : '/img/dark/logo/logo_white.png'}
            alt="Elenchus Logo"
            width={50}
            height={50}
          />
        ...
        <div className="flex items-center space-x-6">
          ...
          <span className="hidden text-sm font-medium text-foreground dark:text-white md:block">
            EN
          </span>
          <Link href="/login" className="hidden text-sm font-medium text-foreground dark:text-white md:block">
            Login
          </Link>
          <Link href="/book-demo">
